...
# === YOUR CODE HERE ===========================================================
      ...
      # new_uv = np.array(points).T
      xy = np.array(points).T

      # for i in range(len(uv[0])):
      #   new_uv[0][i] -= zero[0]
      #   new_uv[1][i] = zero[1] - new_uv[1][i]
      # print(new_uv)
      ...
      for i in range(4):
        vec1 = np.array([xy[0][i],xy[1][i],1,0,0,0,-1 * uv[0][i]*xy[0][i],-1 * uv[0][i]*xy[1][i]])
        vec2 = np.array([0,0,0,xy[0][i],xy[1][i],1,-1 * uv[1][i]*xy[0][i],-1 * uv[1][i]*xy[1][i]])
        A[2*i] = vec1.T
        A[2*i+1] = vec2.T
      ...
      bottom1 = H_inv[2][0]*uv[0][4] + H_inv[2][1]*uv[1][4] + H_inv[2][2]

      xtop1 = H_inv[0][0]*uv[0][4] + H_inv[0][1]*uv[1][4] + H_inv[0][2]
      ytop1 = H_inv[1][0]*uv[0][4] + H_inv[1][1]*uv[1][4] + H_inv[1][2]

      bottom2 = H_inv[2][0]*uv[0][5] + H_inv[2][1]*uv[1][5] + H_inv[2][2]

      xtop2 = H_inv[0][0]*uv[0][5] + H_inv[0][1]*uv[1][5] + H_inv[0][2]
      ytop2 = H_inv[1][0]*uv[0][5] + H_inv[1][1]*uv[1][5] + H_inv[1][2]

      x1 = (xtop1/bottom1,ytop1/bottom1)
      x2 = (xtop2/bottom2,ytop2/bottom2)
